...
# Returns a Velfield collections object. 	
	...
	myVelfield = Velfield(name=name, nlat=nlat, elon=elon, n=n, e=e, u=u, sn=sn, se=sn, su=su, first_epoch=first_epoch, last_epoch=last_epoch);
	[myVelfield] = clean_velfield(myVelfield, 1, .95, [-126, -114, 32, 48]);  # A really wide western US lens
	return [myVelfield]; 

# reads ETS and inter-ETS vel files as generated by Noel Bartlow
def read_ETS_vel_file(infile):
	...
	for line in ifile:
		...
		if len(temp)== 0:
			continue;
		elif temp[0]=="#" or temp[0]=="WSLR":  # WSLR doesn't exist in UNR data
			continue; 
		else:
			name.append(temp[0]);
...
def get_coordinates_for_stations(station_names,coordinates_file="Vel_Data/DataHoldings.txt"):
	lon=[];
	lat=[];
...
def get_start_times_for_stations(station_names,coordinates_file="Vel_Data/DataHoldings.txt"):
...
def remove_blacklist(velfield, blacklist_file):
