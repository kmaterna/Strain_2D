...
output_dir="Output/"
strain1=$output_dir"delaunay/"
strain2=$output_dir"gpsgridder/"
strain3=$output_dir"visr/"
strain4=$output_dir"huang/"
...
gmt psvelo $strain1"positive_eigs.txt" -Se0.003/0.68/0 -A+e+n10+pthick,blue -Gblue -R$range -J$projection -K -O >> $out_strain
gmt psvelo $strain1"negative_eigs.txt" -Se0.003/0.68/0 -A+b+n10+pthick,black -Gred -R$range -J$projection -K -O >> $out_strain
...
gmt psvelo $strain4"positive_eigs.txt" -Se0.003/0.68/0 -A+e+n10+pthick,blue -Gblue -R$range -J$projection -K -O >> $out_strain
gmt psvelo $strain4"negative_eigs.txt" -Se0.003/0.68/0 -A+b+n10+pthick,black -Gred -R$range -J$projection -K -O >> $out_strain
...
gmt psvelo $strain3"positive_eigs.txt" -Se0.003/0.68/0 -A+e+n10+pthick,blue -Gblue -R$range -J$projection -K -O >> $out_strain
gmt psvelo $strain3"negative_eigs.txt" -Se0.003/0.68/0 -A+b+n10+pthick,black -Gred -R$range -J$projection -K -O >> $out_strain

# GPSgridder Strain
gmt grdimage $strain2"I2nd.nc" -R$range -J$projection -BweSn -Bp1.0 -Cmycpt.cpt -X6 -Y0 -K -O >> $out_strain
gmt pscoast -R$range -J$projection -Wthick,black -Df -Sgray -K -O >> $out_strain
echo "-122.8 42.3 gpsgridder" | gmt pstext -R$range -J$projection -F+f18p,Helvetica-bold -N -K -O >> $out_strain
gmt psscale -DjTR+w4.5/0.5+o-1.1/1.5 -R$range -J$projection -B1.0:"log(I2)":/:: -Cmycpt.cpt -K -O >> $out_strain
gmt psvelo $strain2"positive_eigs.txt" -Se0.003/0.68/0 -A+e+n10+pthick,blue -Gblue -R$range -J$projection -K -O >> $out_strain
gmt psvelo $strain2"negative_eigs.txt" -Se0.003/0.68/0 -A+b+n10+pthick,black -Gred -R$range -J$projection -K -O >> $out_strain
